...
print_heatmap <- function(df, label, sequence, output_file = NULL, low = NA, high = NA,
                          order_in = default_order, names = default_variant_names,
                          p1_in = 0.9, p2_in = NA, p3_in = 0.4, p4_in = NA, print = FALSE) {
  
  label <- enquo(label)
  
  sequence_len = max(df$pos)
  strips = ceiling(sequence_len/100)
  ...
  for (row in 0:(strips - 1)) {
    ...
    lower_limit = 1 + (row * 100)
    upper_limit = min(sequence_len, 100 + (row * 100))
    ...
    new_row = ggplot(data = df %>% filter(pos %in% c(lower_limit:upper_limit)) %>%
                       mutate(score = pmin(pmax(!! label, low), high)), 
                     aes(x = pos,
                         y = factor(variants, level = order_in, labels = names),
                         fill = score, height = 0.9, width = 0.9)) +
